...
const columns: ColumnDef<ICashPaymentsAdvance>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) =>
      PAYMENT_OPTIONS.find(
        (option) => option?.id === (row.getValue('type') as unknown),
      )?.name,
  },
  {
    ...
    cell: ({ row }) =>
      dayjs(row.getValue('createdAt')).format('DD/M/YYYY hh:mm A'),
  },
  {
    accessorKey: 'payment.event.licenseType.name',
    header: ({ column }) => {
      return (
        <Button
          className="px-0 font-bold text-base"
  ...
  {
    accessorKey: 'payment.event.customer.name',
    header: ({ column }) => {
      return (
        <Button
          className="px-0 font-bold text-base"
...
export const SalesTable = (props: Props) => {
  ...
  const exportToPDF = () => {
    ...
    const tableData = filterData.map((row) => [
      ...
      PAYMENT_OPTIONS.find((option) => option?.id === (row.type as unknown))
        ?.name || '',
      dayjs(row.createdAt).format('DD/M/YYYY hh:mm A'),
      row.payment?.event?.licenseType?.name || '',
    ...
    autoTable(doc, {
      head: [['Monto', 'Tipo de pago', 'Fecha y hora', 'Tipo', 'Cliente']],
      body: tableData,
      styles: {
  ...
  return (
    <div className="pt-8 mt-8">
      <h1 className="font-medium text-xl text-secondary animate-fade-right animate-once animate-duration-500 animate-delay-100 animate-ease-in">
        Filtros
      </h1>
