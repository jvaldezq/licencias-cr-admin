...
import {usePaymentMutation, useGetEventById} from "@/app/events/services/client";
...
export const PaymentEvent = (props: CompleteEventProps) => {
    ...
    return (<Dialog
        ...
        footer={<Button
            ...
            className="bg-secondary text-white rounded-3xl animate-fade-right animate-once animate-duration-500 animate-delay-100 animate-ease-in">Pagar</Button>}
        trigger={null}
    >
        <EventWrapper id={id} user={user} setOpen={setOpen} setIsLoading={setIsLoading}
...
const EventWrapper = (props: EventWrapperProps) => {
    ...
    const initialValues = {
        type: PAYMENT_TYPE.CASH, amount: amountToPay,
    }
    ...
    >
        {(formProps) => <form id="event-complete-form" onSubmit={formProps.handleSubmit}
            ...
            {
                data?.payment?.cashPaymentsAdvance?.map((payment, index) => <div key={`payment-${index}`} className="flex justify-between items-center gap-2 w-full">
                    <p className="text-sm font-semibold text-primary/[0.9]">Adelanto #{index + 1} ({PAYMENT_OPTIONS.find(option => option?.id === payment.type as unknown)?.name})</p>
            ...
            <div className="w-full flex flex-col gap-4 py-4 border-t border-solid border-primary">
                <Field
                    ...
                    validate={(value) => value !== undefined ? undefined : 'El monto es requerido'}
                />
                <Field
                    name="type"
