...
import { CarChecksFormProps } from '@/app/assets/forms/CarChecks';
...
const completeChecksAsset = async (
  data: CarChecksFormProps,
): Promise<IAsset> => {
  const asset = await clientApi.patch(`/asset/${data.id}/checks`, data);
  return asset.data;
};
...
export const useCompleteChecksMutation = () => {
  return useMutation({
    mutationFn: (data: CarChecksFormProps) => {
      return completeChecksAsset(data);
    },
    mutationKey: ['asset-checks'],
  });
};
